...
首次上传使用 `afterSeq: -1`，并从当前日志的起始处开始。此后每次上传都从同一会话 id 的最大已接受水位（watermark）之后开始。单次请求只接纳待发后缀中按最旧优先的一批——受贡献插件的 `maxBatchBytes` 限制（默认 4 MiB）——并以该批的最后一条序号作为 `throughSeq`，因此超过该限制的积压会跨多次请求排空，同时每条序列仍按顺序送达。发送方为每次请求仅快照一次事件数组；快照后的追加内容属于后续请求。
